...
TEST(EncoderModule, OptimizedEncoding) {
    agk::Log::Init();
    ...
    std::string outputFile = "test_output_optimized.mkv";
...
TEST(EncoderModule, ErrorHandling) {
    agk::Log::Init();
    agk::EncoderModule encoder;
    agk::VideoConfig vConfig;
    agk::AudioConfig aConfig;
    ...
    std::string outputFile = "test_output_error.mkv";
    if (fs::exists(outputFile)) fs::remove(outputFile);

    // Test encoding without initialization
    size_t bufferSize = vConfig.width * vConfig.height * 4;
    std::vector<uint8_t> dummyFrame(bufferSize, 0);
    ASSERT_FALSE(encoder.EncodeVideoFrame(dummyFrame.data(), vConfig.width * 4, 0));

    // Initialize and test with invalid data
    ASSERT_TRUE(encoder.Initialize(outputFile, vConfig, aConfig, nullptr));
    ASSERT_FALSE(encoder.EncodeVideoFrame(nullptr, vConfig.width * 4, 0));
